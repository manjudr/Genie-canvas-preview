function X2JS(t) {
    ...
    function c(t) {
        ...
        if (r.setHours(e[3], e[4], n[0]), n.length > 1 && r.setMilliseconds(n[1]), e[6] && e[7]) {
            ...
            i = 0 + ("-" == (/\d\d-\d\d:\d\d$/.test(t) ? "-" : "+") ? -1 * i : i), r.setMinutes(r.getMinutes() - i - r.getTimezoneOffset());
    ...
    function d(e, r) {
        return !!("property" == t.arrayAccessForm && g(r.toString(), "_asArray") || 0 == r.toString().indexOf(t.attributePrefix) || 0 == r.toString().indexOf("__") || e[r] instanceof Function);
    }
    function x(t) {
        var e = 0;
    ...
    function O(t) {
        var e = "";
        if (x(t) > 0) for (var n in t) if (!d(t, n)) {
            var i = t[n], a = h(i);
...
PluginManager = {
    ...
    registerPlugin: function(id, plugin) {
        org.ekstep.pluginframework.pluginManager._registerPlugin(id, void 0, plugin), "undefined" != typeof createjs && createjs.EventDispatcher.initialize(plugin.prototype);
    },
...
var ItemController = Controller.extend({
    ...
    evalItem: function() {
        try {
            var result, instance = this, item = this.getModel();
            if ("ftb" == item.type.toLowerCase() ? result = FTBEvaluator.evaluate(item) : "mcq" == item.type.toLowerCase() || "mmcq" == item.type.toLowerCase() ? result = MCQEvaluator.evaluate(item) : "mtf" == item.type.toLowerCase() && (result = MTFEvaluator.evaluate(item)), 
            result && (result.pass, item.score = result.score), !_.isUndefined(item.concepts)) var concepts = _.isArray(item.concepts) && _.isObject(item.concepts) ? item.concepts : JSON.parse(item.concepts);
            var data = {
                pass: result.pass,
                score: item.score,
                res: result.res,
    ...
    getMMC: function(item, result) {
        try {
            var mmc = [], obj = {};
            return _.each(result.res, function(each) {
                Object.assign(obj, each);
            }), "string" == typeof item.responses && (item.responses = JSON.parse(item.responses)), 
...
ControllerManager.registerController("items", ItemController), MCQEvaluator = {
    ...
    evaluate: function(item) {
        var result = {}, pass = !0, score = 0, res = [];
        if (item) {
            var options = item.rhs_options, selectedOptions = [];
            if (_.isArray(options) && _.each(options, function(opt) {
                if (void 0 !== opt.selected) {
                    var obj = {};
                    obj[opt.value.resvalue] = opt.value.mapped, res.push(obj), selectedOptions.push(opt);
                }
                void 0 !== opt.answer ? opt.answer == opt.selected && (score += _.isNumber(opt.score) ? opt.score : 1) : void 0 !== opt.selected && (pass = !1);
            }), pass) {
                var ansMatched = _.isEqual(_.pluck(selectedOptions, "selected"), _.pluck(selectedOptions, "answer")) && item.lhs_options.length == selectedOptions.length || !1;
                pass = ansMatched;
            }
            pass || (result.feedback = item.feedback, item.partial_scoring || (score = 0));
        }
    ...
    registerEvent: function(evt, plugin) {
        ...
        if (register) if (plugin.events.push(evt.type), _.contains(createjs.DisplayObject._MOUSE_EVENTS, evt.type)) {
            ...
            element && (plugin instanceof HTMLPlugin || plugin._div ? (element = plugin._self.htmlElement, 
    ...
    setStageConfig: function() {
        ...
        for (i = 0; i < evtLenth; i++) {
            var val, eventName = this._eventsArray[i];
            _.isUndefined(Renderer.theme) || _.isUndefined(Renderer.theme._currentScene) || (val = Renderer.theme._currentScene.getParam(eventName)), 
            _.isUndefined(val) && (_.isUndefined(this._contentConfig[eventName]) || (val = this._contentConfig[eventName])), 
    ...
    handleSubmit: function() {
        ...
        if (!_.isUndefined(Renderer.theme) && _.isUndefined(Renderer.theme.getParam(eventName)) && _.isUndefined(Renderer.theme._currentScene.getParam(eventName))) {
            val = EkstepRendererAPI.getGlobalConfig().overlay.showSubmit ? "on" : "off";
        }
        if (_.isUndefined(Renderer.theme) || _.isUndefined(Renderer.theme._currentScene) || !Renderer.theme._currentScene.isItemScene()) EventBus.dispatch(eventName, "off"); else {
            if ("on" == val) {
    ...
    showFeeback: function(showOverlayGoodJob) {
        var returnVal = !0;
        return showOverlayGoodJob ? (returnVal = "off" != this._stageConfig.overlayGoodJob, 
        this.showGoodJobFb(returnVal)) : (returnVal = "off" != this._stageConfig.overlayTryAgain, 
    ...
    skipAndNavigateNext: function(param) {
        try {
            ...
            if (void 0 === navigateTo) {
                if (_.isUndefined(Renderer.theme._currentScene)) return;
                Renderer.theme._currentScene.isItemScene() && !_.isUndefined(Renderer.theme._currentScene._stageController) && Renderer.theme._currentScene._stageController.hasNext() ? this.defaultNavigation(actionType, navigateTo) : this.moveToEndPage();
    ...
    moveToEndPage: function() {
        console.info("redirecting to endpage."), Renderer.theme._currentStage = void 0, 
        EkstepRendererAPI.dispatchEvent("renderer:telemetry:end"), EkstepRendererAPI.dispatchEvent("renderer:content:end");
        var stage = Renderer.theme._currentScene;
        Renderer.theme.setParam(stage.getStagestateKey(), stage._currentState), EkstepRendererAPI.removeHtmlElements(), 
    ...
    defaultNavigation: function(navType, navigateTo) {
        ...
        navType = "skip" === navType ? "next" : navType, action.transitionType = navType, 
    ...
    actionReplay: function(data) {
        ...
        GlobalContext.currentContentId && version && startTelemetry(GlobalContext.currentContentId, version), 
        EkstepRendererAPI.removeHtmlElements(), Renderer.theme.reRender();
    }
    ...
    init: function(themeData, basePath) {
        ...
        themeData.manifest.media.forEach(function(media) {
            if (media && media.src) {
                if ("http" != media.src.substring(0, 4)) if (isbrowserpreview) {
                    var globalConfig = EkstepRendererAPI.getGlobalConfig();
                    media.src = globalConfig.host + media.src;
                } else media.src = basePath + media.src;
                if (createjs.CordovaAudioPlugin.isSupported() && "sound" !== media.type && "audiosprite" !== media.type && (media.src = "file:///" + media.src), 
    ...
    loadStage: function(stageId, callback) {
        ...
        if (!instance.loaders[stageId]) {
            ...
            }), mediaList = instance.filterMedia(mediaList, "video"), _.isArray(mediaList) && mediaList.length > 0) {
    ...
    loadCommonAssets: function() {
        ...
        this.commonAssets = this.filterMedia(this.commonAssets, "video"), loader.loadManifest(this.commonAssets, !0), 
    ...
    loadTemplateAssets: function() {
        var loader = this._createLoader();
        loader.setMaxConnections(this.MAX_CONNECTIONS), this.templateAssets = this.filterMedia(this.templateAssets, "video"), 
        loader.installPlugin(createjs.Sound), loader.loadManifest(this.templateAssets, !0), 
        loader.on("error", function(evt) {
    ...
    isStageAssetsLoaded: function(stageId) {
        ...
        return !(_.isUndefined(this.loaders[stageId]) || !this.loaders[stageId].stageLoaded) || !!_.isUndefined(this.loaders[stageId]);
    }
}), RecorderManager = {
...
var DataGenerator = {
    ...
    _callBack: function(response) {
        _(Renderer.theme).isUndefined() || void 0 !== response && "success" == response.status && this._invokeRelatedActions("children");
    }
    ...
    evaluate: function(action) {
        ...
        if (!1 !== this._isStageStateChanged || !isEvaluated) {
            ...
            if (this._stageController) {
                ...
                this.isStageStateChanged(!1), showImmediateFeedback) {
                    if (1 == valid) {
...
var ThemePlugin = Plugin.extend({
    ...
    render: function() {
        ...
        }), this.invokeStage(this._data.startStage)), this.update(), console.log("ECML RENDERER - Content rendering started"), 
    ...
    restart: function() {
        var gameId = TelemetryService.getGameId(), version = TelemetryService.getGameVer(), instance = this, telemetryEndData = {};
        telemetryEndData.stageid = getCurrentStageId(), telemetryEndData.progress = logContentProgress(), 
        TelemetryService.end(telemetryEndData), AssetManager.initStage(this._data.startStage, null, null, function() {
    ...
    transitionTo: function(action) {
        if (!this._isSceneChanging) {
            ...
            "skip" === action.transitionType ? this.jumpToStage(action) : (this._isSceneChanging = !0, 
            "previous" === action.transitionType ? stage._stageController && stage._stageController.hasPrevious() ? (stage._stageController.decrIndex(2), 
            this.replaceStage(stage._data.id, action)) : (stage._stageController && (stage._stageController.setIndex(-1), 
            1 == action.reset && stage._stageController.reset()), this.replaceStage(action.value, action)) : "next" === action.transitionType && stage._stageController && stage._stageController.hasNext() ? this.replaceStage(stage._data.id, action) : this.jumpToStage(action));
        }
    },
    jumpToStage: function(action) {
        stage._stageController && action.reset && stage._stageController.reset(), this.replaceStage(action.value, action);
    },
...
var VideoPlugin = Plugin.extend({
    ...
    initPlugin: function(data) {
        ...
        _instance = this, _instance.loadVideo(data);
    },
    loadVideo: function(data) {
        if (!data.asset) return !1;
        var lItem = this.createVideoElement();
        this.getVideo().load(), this.registerEvents(), this._self = new createjs.Bitmap(lItem), 
    ...
    registerEvents: function() {
        ...
        jQuery(videoEle).bind("loadeddata", this.onLoadData), jQuery(videoEle).bind("ended", this.showReplay), 
    ...
    getVideo: function(action) {
        return _.isUndefined(action) ? document.getElementById(this._data.asset) : document.getElementById(action.asset);
    },
    ...
    addVideoElement: function(jqVideoEle) {
        ...
        var videoEle = this.getVideo(), div = document.getElementById("gameArea");
    ...
    showReplay: function(event) {
        try {
            var img = (_instance.getRelativeDims(org.ekstep.pluginframework.pluginManager.pluginInstances[event.target.id]._data), 
            document.createElement("img")), replay_id = "replay_" + event.target.id;
            jQuery(img).attr({
                src: _instance._replayIcon,
                id: replay_id
            }), _instance.disableBackground(event.target.id, !0), _.isNull(document.getElementById(replay_id)) && jQuery(img).insertAfter("#" + _instance.id), 
            window.screenTop || window.screenY ? _instance.onNormalScreen(event) : _instance.onFullScreen(event), 
    ...
    hideReplay: function(event) {
        ...
        _instance.disableBackground(vidoeId, !1), document.getElementById(vidoeId).play(), 
    ...
    onFullScreen: function(event) {
        var replay_id = "replay_" + event.target.id, element = document.getElementById(event.target.id), positionInfo = element.getBoundingClientRect();
    ...
    onNormalScreen: function(event) {
        var replay_id = "replay_" + event.target.id, dims = _instance.getRelativeDims(org.ekstep.pluginframework.pluginManager.pluginInstances[event.target.id]._data), top = dims.y + (dims.h / 2 - 40) + "px", left = dims.x + (dims.w / 2 - 30) + "px";
        _instance.setReplayIconStyle(replay_id, {
            width: "50px",
            height: "50px",
            "z-index": "1",
    ...
    muteAll: function() {
        ...
        videoElements.length > 0 && _.each(videoElements, function(videoElem) {
            videoElem.muted = !0;
        });
    },
    unmuteAll: function() {
...
android.recorder = {
    start: function(path) {
        return new Promise(function(resolve, reject) {
            ...
            if ("undefined" != typeof Media) {
                ...
                media.startRecord(), result.media = media, result.status = "success";
            } else result.status = "success", result.errMessage = "Media is not available.", 
    ...
    stop: function(instance) {
        return new Promise(function(resolve, reject) {
            var result = {};
            "undefined" != typeof Media ? instance && instance.media ? (instance.media.stopRecord(), 
            instance.media.release(), result = {
                status: "success"
            }) : result = {
                status: "ERROR",
                errMessage: "Error recording not started."
            } : (console.info("AndroidRecorder.stopRecording called."), result = {
                status: "success",
    ...
    startRecording: function(path, cb) {
        speech.recording = !1, speech.getRecorder().start(path).then(function(mediaInstance) {
            speech.mediaInstance = mediaInstance, speech.mediaInstance && "success" == speech.mediaInstance.status && (speech.recording = !0, 
            speech.mediaInstance.filePath = path), cb(mediaInstance);
        }).catch(function(err) {
            console.error("Error start recording audio:", err), cb({
                status: "error",
                error: err
            });
        });
    ...
    stopRecording: function(cb) {
        speech.recording && speech.mediaInstance ? speech.getRecorder().stop(speech.mediaInstance).then(function(response) {
            "success" == response.status ? (speech.recording = !1, console.info("Audio file saved at ", speech.mediaInstance.filePath), 
            cb(speech.mediaInstance)) : cb(response);
        }).catch(function(err) {
            console.error("Error stop recording audio:", err), cb({
                status: "error",
                error: err
            });
        }) : cb({
    ...
    processRecording: function(lineindex, filePath, cb) {
        (filePath = filePath || (speech.mediaInstance ? speech.mediaInstance.filePath : "")) && lineindex && speech.getRecorder().process(filePath, lineindex).then(function(response) {
            speech.mediaInstance = void 0, cb(response);
        }).catch(function(err) {
            console.error("Error processing audio:", err), cb({
                status: "error",
                error: err
            });
        });
